...
namespace Creational.AsyncToSync
{
    public class AsyncToSync
    {
        public static T RunWithoutContext<T>(Func<Task<T>> factory)
        {
            ...
            return task.ConfigureAwait(false) //continueOnCapturedContext = false
                //configures the task so that continuation after the await does not have to be run 
                //in the caller context, therefore avoiding any possible deadlocks.
                //false in details make the system not to perform 
                //.Post (System.Threading.SendOrPostCallback d, object state);
                //on captured instance of SyncronisationContext, instead the rest of method 
                //after await will be executed by any thead 
                .GetAwaiter()
                .GetResult();
        }
    }
